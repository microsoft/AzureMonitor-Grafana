  ...
  const workloadLinks: DataLink[] = [
    {
      ...
      url: getDataLink("workload/computeresources", `var-${NS_VARIABLE}=\${__data.fields.namespace}&var-${WORKLOAD_VAR}=\${__data.fields.workload}&\${${PROM_DS_VARIABLE}:queryparam}&\${${AZMON_DS_VARIABLE}:queryparam}&\${${CLUSTER_VARIABLE}:queryparam}&\${${SUBSCRIPTION_VARIABLE}:queryparam}`),
      targetBlank: false
    }
  ];
  ...
  const alertLinks: DataLink[] = [
    {
      ...
      url: getDataLink("workloads/alertsummary/\${__data.fields.namespace}", `\${${SUBSCRIPTION_VARIABLE}:queryparam}&\${${AZMON_DS_VARIABLE}:queryparam}&\${${CLUSTER_VARIABLE}:queryparam}`),
      targetBlank: false
    }
  ];
  switch(name) {
