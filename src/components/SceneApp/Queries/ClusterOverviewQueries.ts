...
import { AZMON_DS_VARIABLE, CLUSTER_VARIABLE, NS_VARIABLE, PROM_DS_VARIABLE, SUBSCRIPTION_VARIABLE, VAR_ALL } from '../../../constants';
  ...
  const alertLinks: DataLink[] = [
    {
      ...
      url: getDataLink("namespaces/alertsummary/\${__data.fields.namespace}",`\${${SUBSCRIPTION_VARIABLE}:queryparam}&\${${AZMON_DS_VARIABLE}:queryparam}&\${${CLUSTER_VARIABLE}:queryparam}`),
      targetBlank: false
    }
  ];
  ...
  const namespaceLinks: DataLink[] = [
    {
      ...
      url: getDataLink("workloads", `var-${NS_VARIABLE}=\${__data.fields.namespace}&var-${SUBSCRIPTION_VARIABLE}=${subscriptionId}&\${${CLUSTER_VARIABLE}:queryparam}&\${${AZMON_DS_VARIABLE}:queryparam}`),
      targetBlank: false
    }
  ]
  switch(name) {
